...
public class BasicImageUI extends ImageUI implements MouseListener, MouseMotionListener {
  ...
  public void installUI(JComponent c) {
    ...
    rangeSelectionManagerChangedListener = new RangeSelectionManagerChangedListener(rangeSelectionManager, imageView, minSelectionIndex, maxSelectionIndex, imageComponent);
    rangeSelectionManager.addRangeSelectionManagerChangedListener(rangeSelectionManagerChangedListener);
      ...
      highlightColorSelectionModelChangeListener = new HighlightColorSelectionModelChangeListener(imageComponent);
...
//System.out.println("BasicImageUI.paint");
    ...
    for (int line = linesAbove; line < endLine; line++) {
      ...
      g.setColor(highlightColorSelectionModel.getSelectedColor());
...
//WLog.log("BasicImageUI.mouseReleased: " + minSelectionIndex);
  ...
  private class ImageViewChangedListener implements GenericPropertyChangeListener<ImageView.ChangeType> {

      @Override
      public void propertyChange(PropertyChangeEvent evt, ImageView.ChangeType oldValue, ImageView.ChangeType changeType) {

          switch (changeType) {
              case IMAGE_PAGE_CHANGED:
              case LINE_FORMAT_CHANGED:
                  // With these changes, the content of the lines changes,
                  ...
                  // so we must repaint whether scrollpane did so or not.
                  imageComponent.repaint();
                  break;

              case USER_HIGHLIGHT_CHANGED:
                  // Although highlighting changed, geometry did not change.
                  imageComponent.repaint();
                  break;

              case FORENSIC_PATH_NUMERIC_BASE_CHANGED:
