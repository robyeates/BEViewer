...
public class WScanProgress extends JDialog {
        ...
        public void run() {
            ...
            if (leftParenIndex > 0 && rightParenIndex > leftParenIndex) {
                ...
                try {
                    ...
                    progressBar.setString(progressFloat + "%");
                } catch (NumberFormatException e) {
                    logger.error("WScanProgress.run: unexpected progress value '{}' in stdout: {}", progress, input);
                }
    ...
    private class RunnableShowStderr implements Runnable {
        ...
        public void run() {
            ...
            logger.error("bulk_extractor scan error: '{}'", input);
            outputArea.append(input);
            outputArea.append("\n");
        }
    }
    ...
    private class RunnableSetDoneState implements Runnable {
        public void run() {
            ...
            if (exitValue == 0) {
                // successful run
                statusL.setText("bulk_extractor scan completed.  Report " + new File(scanSettings.outdir).getName() + " is ready.");
                progressBar.setValue(100);
                progressBar.setString("Done");
                // add the report that has been generated by this run
                DaggerContext.get().reportsModel().addReport(new File(scanSettings.outdir), new File(scanSettings.inputImage));

            } else if (exitValue == 143) {
                // canceled run
                statusL.setText("The bulk_extractor scan was canceled");
                logger.info("bulk_extractor scan canceled");
                progressBar.setString("Canceled");
                WError.showError("bulk_extractor Scanner canceled." + "\n" + scanSettings.getCommandString(), "bulk_extractor scan canceled", null);

            } else {
                // failed run
                statusL.setText("Error: bulk_extractor terminated with exit value " + exitValue + ".  Please check command syntax.");
                logger.error("bulk_extractor error exit value: {}", exitValue);
                progressBar.setString("Error");
                WError.showError("bulk_extractor Scanner terminated with exit value " + exitValue + ".  Please check command syntax:" + "\n" + scanSettings.getCommandString(), "bulk_extractor scan terminated", null);
            }
        }
    }
    ...
    private class ThreadStdout extends Thread {
        ...
        ThreadStdout() {
            readFromStdout = new BufferedReader(new InputStreamReader(process.getInputStream()));
        }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    logger.error("ScanSettingsConsumer.ThreadStdout.run aborting.");
                    break;
                }
            }
        }
    ...
    private class ThreadStderr extends Thread {
        ...
        ThreadStderr() {
            readFromStderr = new BufferedReader(new InputStreamReader(process.getErrorStream()));
        }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    logger.error("ScanSettingsConsumer.ThreadStderr.run aborting.");
                    break;
                }
            }
        }
    ...
    private record RunnableWScanProgress(ScanSettings scanSettings, Process process) implements Runnable {
...
//    c.anchor = GridBagConstraints.FIRST_LINE_START;
    ...
    private void doCancel() {
        if (isAlive(process)) {
            ...
            logger.info("WScanProgress.doCancel: process already canceled");
        }
    }
