...
public class ReportSelectionManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ReportSelectionManager.class);
    ...
    private final ChangeType changeType = ChangeType.SELECTION_CHANGED;
    ...
    public void setReportSelection(final File imageFile, final File featuresFile) {
        ...
        logger.info("ReportSelectionManager.setReportSelection image file: {}, features file: {}, selection type: {}", imageFile, featuresFile, reportSelectionType);

        // notify
        propertyChangeSupport.firePropertyChange(ChangeType.SELECTION_CHANGED.name, null, featuresFile);
    }
    ...
    public void setRequestHideReferencedFeatureView(boolean requestHideReferencedFeatureView) {
        if (this.requestHideReferencedFeatureView != requestHideReferencedFeatureView) {
            var old = this.requestHideReferencedFeatureView;
            this.requestHideReferencedFeatureView = requestHideReferencedFeatureView;
            propertyChangeSupport.firePropertyChange(ChangeType.PREFERENCE_CHANGED.name, old, requestHideReferencedFeatureView);
        }
    }
     ...
     */
    public void addReportSelectionManagerChangedListener(PropertyChangeListener reportSelectionManagerChangedListener) {
        propertyChangeSupport.addPropertyChangeListener(reportSelectionManagerChangedListener);
    }
     ...
     */
    public void removeReportSelectionManagerChangedListener(PropertyChangeListener reportSelectionManagerChangedListener) {
        propertyChangeSupport.removePropertyChangeListener(reportSelectionManagerChangedListener);
    }
    ...
    private boolean isHistogramFile(final File file) {
        ...
        try {
            ...
            if (bom[0] == utf8BOM[0] && bom[1] == utf8BOM[1] && bom[2] == utf8BOM[2]) {
                ...
                logger.warn("ReportSelectionManager.isHistogramFile: Note: File {} does not contain a UTF-8 Byte Order Mark.", file);
            }
     ...
     */
    public record ReportSelectionType(String name) {
