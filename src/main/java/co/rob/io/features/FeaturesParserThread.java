 ...
 */
@Deprecated(since = "This god class is preserved solely for A/B testing against the decomposed version")
public class FeaturesParserThread extends Thread {


    private static final Logger logger = LoggerFactory.getLogger(FeaturesParserThread.class);
  ...
  private FastFeatureLineTable featureLineTable;
  ...
  public FeaturesParserThread(FeaturesModel callback, File imageFile, File featuresFile,
    ...
    featureLineTable = new FastFeatureLineTable(imageFile, featuresFile);
  ...
  private void parseFile() {
    ...
    if (requestedFilterBytes.length == 0) {
      ...
      if (filterMatchCase == true) {
        ...
        filter = UTF8Tools.asciiToLower(requestedFilterBytes);
      }
    ...
    try {
      ...
      fileInputStream = null;
        ...
        logger.error("FeaturesModel.parseFile unable to close fileInputStream");
    }
    try {
      fileChannel.close();
      fileChannel = null;
    } catch (IOException ioe2) {
        logger.error("FeaturesModel.parseFile unable to close fileChannel");
    }
  } // parseFile()
...
//    throw new RuntimeException("flow error");
  ...
  private void commitChange(FastFeatureLineTable featureLineTable) {
  ...
  private static class DoCallback implements Runnable {
    private final FeaturesModel callback;
    private final FastFeatureLineTable featureLineTable;
    public DoCallback(FeaturesModel callback, FastFeatureLineTable featureLineTable) {
      this.callback = callback;
